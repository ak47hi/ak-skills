...
System_Boundary(oms, "Order Management System") {
    ...
    Container(api, "API", "Go, gRPC", "Edge API; authn/authz, request routing.")
    ...
    ContainerDb(db, "Postgres", "Postgres", "Customers, orders, payment refs.")
...
Rel(orders, sendgrid, "Sends order email via", "HTTPS")
...
C4 Container view of the order-management system: a Customer using the React Web app talks to the Go gRPC API, which routes to AuthService and OrderService over gRPC; both services share a Postgres DB, OrderService publishes domain events to an SQS event bus and integrates with external Stripe (payments) and SendGrid (transactional email).
