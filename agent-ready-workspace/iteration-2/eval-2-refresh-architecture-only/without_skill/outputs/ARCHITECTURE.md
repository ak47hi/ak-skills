...
> Last refreshed: 2026-05-25 (sourced from `.understand-anything/knowledge-graph.json`, `analyzedAt: 2026-05-25`).
>
> Mermaid blocks below have **not** been rendered by `mmdc` in this refresh — render them in your viewer (GitHub renders Mermaid natively) or install `@mermaid-js/mermaid-cli` and run `mmdc -i ARCHITECTURE.md` to validate syntactically.
...
1. **Coverage config points at the wrong package.** `pyproject.toml:86` has `source = ["jinja2", "tests"]` under `[tool.coverage.run]` — a copy-paste leak from a sibling Pallets project. No workflow under `.github/workflows/` invokes coverage so CI is unaffected, but local `pytest --cov` reports nothing for `itsdangerous`. See `AGENT_DESIGN_DOC.md` §10 for full context.
