...
C4 Container view of the Order Management System: Customer hits the React Web App, which calls the Go gRPC API Gateway; the gateway fans out to AuthService and OrderService (both Go), backed by Postgres and an SQS event bus, with Stripe and SendGrid as external integrations.
