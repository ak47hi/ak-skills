...
sequenceDiagram
    ...
    participant tsigner as TimestampSigner
    ...
    ser->>tsigner: make_signer(salt).sign(payload)
    tsigner->>tsigner: append base64 of uint64 now
    tsigner->>alg: get_signature(key, value)
    alg-->>tsigner: HMAC-SHA1 digest
    tsigner-->>ser: payload + sep + timestamp + sep + sig
...
sequenceDiagram
    ...
    ser->>tsigner: iter_unsigners then signer.unsign(token, max_age, return_timestamp-true)
    tsigner->>tsigner: rsplit on sep then verify HMAC then check age
    alt age greater than max_age
        tsigner-->>ser: raise SignatureExpired
        ser-->>user: SignatureExpired
    else valid
        tsigner-->>ser: (payload_bytes, datetime)
