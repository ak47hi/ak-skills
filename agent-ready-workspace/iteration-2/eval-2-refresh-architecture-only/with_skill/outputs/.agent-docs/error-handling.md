...
**Catch sites:**
- `base64_decode` wraps `TypeError`/`ValueError` into `BadData("Invalid base64-encoded data")` (`src/itsdangerous/encoding.py:35-38`).
...
- `Signer.verify_signature` swallows base64-decode exceptions and returns `False` rather than propagating (`src/itsdangerous/signer.py:229-232`).
...
- `URLSafeSerializerMixin.load_payload` wraps base64-decode failures into `BadPayload(original_error=e)` (`src/itsdangerous/url_safe.py:36-42`), and zlib-decompress failures separately (`src/itsdangerous/url_safe.py:44-51`).
...
- `TimestampSigner.unsign` wraps `ValueError`/`OSError`/`OverflowError` from `datetime.fromtimestamp` into `BadTimeSignature("Malformed timestamp")` — covers Windows `OSError` and 32-bit `OverflowError` (`src/itsdangerous/timed.py:120-128`).
