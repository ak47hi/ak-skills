...
10. **`SignatureExpired` is NOT swallowed by `TimedSerializer.loads` fallback loop** — once a signature unsigns successfully but is expired, the exception is re-raised immediately; no further signer is tried (`src/itsdangerous/timed.py:213-216`).
...
16. **`base64_decode` silently drops non-ASCII characters** from the input (via `want_bytes(..., errors="ignore")`) before re-adding padding — non-ASCII tokens won't raise, they'll just fail signature verification (`src/itsdangerous/encoding.py:32-33`).
17. **`NoneAlgorithm` returns an empty signature** — explicitly opting in disables tamper detection entirely (`src/itsdangerous/signer.py:31-37`).
